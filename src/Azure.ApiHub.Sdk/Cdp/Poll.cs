        ...
        public async Task Run(Uri pollUri)
        {
            while (!_cancel.IsCancellationRequested)
            {
                ...
                else
                {
                    ...
                    _cdpHelper.Logger.Warning(string.Format("Unexpected status code: {0}, retry after {1} seconds for uri: {2}", response.StatusCode, delay.TotalSeconds, pollUri != null ? pollUri.AbsoluteUri : string.Empty));
